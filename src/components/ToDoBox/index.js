...
function TaskItem({ task, taskId, ...props }) {
  ...
  return (
    <li draggable={true} key={taskId} title={task.deadline.toString()} className='task-item w-full flex items-start bg-lime-50 p-3 mb-2 rounded-lg'>
      ...
      <button onClick={() => deleteTask(taskId)} className='rounded-lg hover:bg-red-600 bg-red-500 text-white px-1.5 pt-1 pb-1.5 text-xs ml-1'><FontAwesomeIcon icon={faTrash} /></button>
    </li>
  )
}
